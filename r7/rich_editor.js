...
RE.callback = function () {
    ...
    RE.inputEventTimerId = setTimeout(function () {
        window.location.href = "re-callback://" + encodeURI(RE.getText().replace(/\+/g, '%2b'));
...
RE.setBold = function () {
    ...
    if (formatBlock.length > 0 && (formatBlock.toLowerCase() == "h1" || formatBlock.toLowerCase() == "h2")) {
        ...
        let ss = document.execCommand('bold', false, null);
...
RE.insertImage = function (url, uuid) {
    var html = '<img id=' + uuid + ' style="max-width:90%;display:block;padding:3px;margin:auto;" src="' + url + '"/>';
    RE.insertHTML(html);
    //var y = RE.getYCaretInfo().y;
    //window.scrollTo(0, y);
}
...
RE.insertBeforeCheck = function (sel) {  //插入之前检测
    var range = sel.range;
    var selElem = RE.getSelectionContainerElem(range);
...
RE.insertGame = function (id, html) {
    var range = RE.insertBeforeCheck(RE.currentSelection);
    var selElem = RE.getSelectionContainerElem();
    //判断选区是否在首行首位   
