...
AndroidDelegate.onGetImagesCount = function(imageCount) {};
...
ZSSEditor.getSelectedText = function() {
	var selection = window.getSelection();
    AndroidDelegate.onGetSelectedText(selection.toString());
	return selection.toString();
};

ZSSEditor.jumpAfterNode = function(el) {
	var range = document.createRange();
...
ZSSEditor.getCaretYPosition = function() {
    if (isMarkdown) {
        return getCaretCoordinates(Maleskine._contentMD[0], Maleskine._contentMD[0].selectionEnd).top;
    } else {
        var selection = window.getSelection();
...
ZSSEditor.getYCaretInfo = function() {
    ...
    var offsetTop = $("#zss_field_content")[0].offsetTop;
...
ZSSEditor.setStrikeThrough = function() {
	if (isMarkdown) {
		...
		//
		var mustHandleWebKitIssue = (isDisablingStrikeThrough && ZSSEditor.isCommandEnabled(commandName));
		...
		if (mustHandleWebKitIssue) {
			...
			while (container && !nodeFound) {
				nodeFound = (container && container.nodeType == document.ELEMENT_NODE && troublesomeNodeNames.indexOf(container.nodeName.toLowerCase()) > -1);
...
ZSSEditor.updateLink = function(url, title) {
	...
	if (isMarkdown) return;
		...
		if (currentLinkNode) {
			...
			ZSSEditor.jumpAfterNode(currentLinkNode);
...
ZSSEditor.MarkdownImageObject.prototype.updateMarkdown = function() {
	if (this.state === 0) {
		...
		this.markdown = '![图片发自简书App](' + this.display + ')';
...
ZSSEditor.MarkdownImageObject.replace = function (editor, url, markdown) {
	...
	while (found) {
		...
		content.replace(/!\[.*?\]\((.*?)\)/i, function(match, image, pos) {
			...
			location += pos;
			if (image === url) {
				newContent = newContent.substring(0, location) + markdown + newContent.substring(location + len);
				if (location < caretPos) {
					shouldResetCaretPos = true;
				}
				location += newLen;
				delta += newLen - len;
			} else {
	...
	if (shouldResetCaretPos) {
    	caretPos = caretPos + delta;
	}
...
ZSSEditor.insertLocalImage = function(imageNodeIdentifier, localImageUrl) {
	...
	if (isMarkdown) {
		...
		var html = imgContainerStart + hiddenChar + progress + hiddenChar + image + hiddenChar + '<br>' + hiddenChar + caption + hiddenChar + input + hiddenChar + imgContainerEnd + "<br />";
		html = hiddenChar + html;
	...
	ZSSEditor.backupRange();
	var y = ZSSEditor.getYCaretInfo().y;
	window.scrollTo(0, y);
};

//批量插入图片
ZSSEditor.insertLocalImages = function(imageNodeIdentifiers, localImageUrls) {
	imageNodeIdentifiers = imageNodeIdentifiers.split(',');
	localImageUrls = localImageUrls.split(',');
	ZSSEditor.restoreRange();
	if (isMarkdown) {
		var allhtml = "";
		for (var i = 0; i < imageNodeIdentifiers.length; i++) {
			var imageNodeIdentifier = imageNodeIdentifiers[i];
			var localImageUrl = localImageUrls[i];
			var mdi = new ZSSEditor.MarkdownImageObject(imageNodeIdentifier);
			mdi.local = localImageUrl;
			mdi.updateMarkdown();
			allhtml += mdi.markdown + "\n"
		}
		ZSSEditor.insertHTML(allhtml);
	}
...
ZSSEditor.replaceLocalImageWithRemoteImage = function(imageNodeIdentifier, remoteImageUrl, displayUrl, slug) {
	if (isMarkdown) {
		if (isPreview) {
			ZSSEditor.markdownPreviewImageUploadSuccessQueue.push({id:imageNodeIdentifier, argv0:remoteImageUrl, argv1:displayUrl, argv2:slug});
			return;
		}
		...
		if (mdi) {
		    ...
		    if (ZSSEditor.eventListeners.log) {
            	ZSSEditor.callback('callback-log', "image-not-found*" + imageNodeIdentifier + "*" + remoteImageUrl);
            }
		}
...
ZSSEditor.setProgressOnImage = function(imageNodeIdentifier, progress) {
	if (isMarkdown) {
		if (isPreview) {
			ZSSEditor.markdownPreviewImageUploadProgressQueue[imageNodeIdentifier] = progress;
			return;
		}
		var mdi = Maleskine.imageCaches[imageNodeIdentifier];
...
ZSSEditor.markImageUploadFailed = function(imageNodeIdentifier, message) {
	if (isMarkdown) {
		if (isPreview) {
			ZSSEditor.markdownPreviewImageUploadFailedQueue.push({id:imageNodeIdentifier, argv:message});
			return;
		}
		var mdi = Maleskine.imageCaches[imageNodeIdentifier];
		mdi.message = message;
		...
		if (imageContainerNode.length != 0) {
			...
			imageContainerNode.find('.image-caption').html('图片上传失败，请删掉重试');
			imageContainerNode.find('.image-caption-input').val('图片上传失败，请删掉重试');
		}
...
ZSSEditor.markdownPreviewImageUploadFailedQueue = [];
ZSSEditor.markdownPreviewImageUploadProgressQueue = {};
ZSSEditor.markdownPreviewImageUploadSuccessQueue = [];
/**
...
ZSSEditor.getImagesCount = function () {
	var imageCount = 0;
    if (isMarkdown) {
        imageCount = Maleskine.content().val().match(/!\[.*?\]\((.*?)\)/g).length;
    } else {
        var imageElements = $('.image-package img');
        imageCount = imageElements.length;
    }
    console.log(imageCount);
    AndroidDelegate.onGetImagesCount(imageCount);
};
...
ZSSEditor.closerParentNodeStartingAtNode = function(nodeName, startingNode) {
	...
	while (currentNode) {
		...
		if (currentNode.nodeName.toLowerCase() == nodeName && currentNode.nodeType == document.ELEMENT_NODE) {
			parentNode = currentNode;
	...
	if (isTitle) {
		if (ZSSEditor.isAndroidHighLevel || !ZSSEditor.isAndroid) {
			ui.bind('keydown', function(e) {
				thisObj.handleKeyDownEventForTitle(e);
			});
		}
	} else {
		if (ZSSEditor.isAndroidHighLevel || !ZSSEditor.isAndroid) {
		...
		ui.bind('compositionstart', function(e) {
			thisObj.handleComposingStartEvent(e);
		});
		ui.bind('compositionend', function(e) {
			thisObj.handleComposingEndEvent(e);
		});
	}
	if (isMarkdown) {
		ui.bind('input', function(e) {
			thisObj.handleInputEventForMarkdown(e);
			thisObj.handleInputEvent(e);
		});
	} else if (isTitle) {
		ui.bind('input', function(e) {
			thisObj.handleInputEventForTitle(e);
			thisObj.handleInputEvent(e);
		});
	} else {
		ui.bind('input', function(e) {
			thisObj.handleInputEventForImageCaption(e);
			thisObj.handleInputEvent(e);
		});
	}
	ui.bind('paste', function(e) {
		thisObj.handlePasteEvent(e);
	});
...
ZSSField.prototype.handleInputEvent = function(e) {
	...
	if (ZSSEditor.eventListeners.input) {
		...
		/**
		* 解决【小米手机一直按住删除键会卡住不能动】的问题，经测试造成卡顿的原因是当用户一直按住删除键时，
			webView就会频繁地产生调用：ZSSEditor.callback("callback-input")，从而导致卡住
		*/
		if (Maleskine.inputEventTimerId) {
			clearTimeout(Maleskine.inputEventTimerId);
		}
		Maleskine.inputEventTimerId = setTimeout(function() {
		var joinedArguments = ZSSEditor.getJoinedFocusedFieldIdAndCaretArguments();
//	    	ZSSEditor.callback('callback-selection-changed', joinedArguments);
			ZSSEditor.callback("callback-input", joinedArguments);
		}, Maleskine.wordageRequestInterval);
...
ZSSField.prototype.handleTapEvent = function(e) {
	...
	if (targetNode) {
		...
		if (targetNode.nodeName.toLowerCase() == 'a') {
			...
			//
			if (ZSSEditor.eventListeners.selectionChanged) setTimeout(function() {
...
ZSSField.prototype.sendImageTappedCallback = function(imageNode) {
	...
	if (container.className.indexOf('img_container') >= 0) {
		...
		//
		if (ZSSEditor.eventListeners.selectionChanged) setTimeout(function() {
			thisObj.callback('callback-image-tap', joinedArguments);
		}, 0);
	} else {
...
Maleskine.focusOnContent = function() {
    if (isFirstLoad) {
        isFirstLoad = false;
        if (isMarkdown) {
    ...
    //打开文章 取消focus on the end of article
    //        window.setTimeout(function() {
    //            var intViewportHeight = window.innerHeight;
    //            var scrollYOffset = ZSSEditor.getYCaretInfo().y;
    //            ZSSEditor.log("intViewportHeight:" + intViewportHeight + ", scrollYOffset:" + scrollYOffset);
    //            if (scrollYOffset > intViewportHeight) {
...
Maleskine.getMarkdownLinkAndImageMD = function(bra, ket, mid, pro, post) {
	...
	var pair_ready = [],
		...
		pair_link = [],
		pair_image = [];
	var start = bra.length - pro.length,
		part_length = pro.length,
		len = pro.length - 1;
...
Maleskine.getImageStates = function() {
	...
	if (isMarkdown) {
		...
		content = content.replace(/!\[.*?\]\((.*?)\)/gmi, function(match, url) {
			...
			/**
			* 如果在上传图片的过程中，杀掉进程，那么![](url)里的url还是指向本地路径
			  重新进入编辑器发布文章的时候，要检查这个url是否合法。在图片上传成功以后，
			  会把本地的file地址替换成上传成功以后的在线地址，所以如果再次打开编辑器的时候，
			  如果Url不是以http/https开头的话，说明图片就没有上传成功
			*/
			else if (url != null && url.substring(0,4) !== "http") {
			    result.failed.push(images.map[url]);
			} else {
		...
		images.each(function(index, img) {
            ...
            /**
            * 在getContent的时候，img的部分class(比如failed)会被移除，所以杀掉进程重新进入编辑器之后，
            图片的状态信息丢失，但是img.parentNote (div)里面保存了当前图片是否上传成功的信息，如果失败了
            就会add data-failed 这个属性，因此在判断图片是否上传成功时，用了parent Node的data-failed来作为
            补充条件
            */
			if (img.classList.contains('failed') || imgParent.getAttribute("data-failed")) {
...
Maleskine.getContent = function(callback) {
	if (isMarkdown) {
		return Maleskine.getMarkdownContent(callback);
	} else {
		return Maleskine.getRichtextContent(callback);
	}
};
Maleskine.getRichtextContent = function(callback) {
	...
	container.find('.image-package').each(function(index, pack) {
		...
		if (src.length == 0) { // Remove if no image.
			...
			pack.html('<img name="' + name + '" src="' + src + '"><br><div class="image-caption">' + caption + '</div>');
		}
	});
	// For Video-Package
	...
	if (ZSSEditor.isAndroid && callback) {
	    AndroidDelegate.getContent(content);
	}
	return content;
};


Maleskine.getMarkdownContent = function(callback) {
	var content = Maleskine.content().val();

	// Remove Emoji
	content = content.replace(Maleskine.RegEx.EMOJI, '');

	if (ZSSEditor.isAndroid && callback) AndroidDelegate.getContent(content);
	return content;
};
...
//			console.log("localUrl " + localURL + " remoteURL " + remoteURL + " displayURL " + displayURL);
//			var found;
...
Maleskine.exitPreviewMode = function() {
	...
	isPreview = false;
	for (var key in ZSSEditor.markdownPreviewImageUploadProgressQueue) {
	    var progress = ZSSEditor.markdownPreviewImageUploadProgressQueue[key];
	    ZSSEditor.setProgressOnImage(key, progress);
	}
	while (ZSSEditor.markdownPreviewImageUploadFailedQueue.length > 0) {
		var info = ZSSEditor.markdownPreviewImageUploadFailedQueue.shift();
		ZSSEditor.markImageUploadFailed(info.id, info.argv);
	}
	while (ZSSEditor.markdownPreviewImageUploadSuccessQueue.length > 0) {
		var info = ZSSEditor.markdownPreviewImageUploadSuccessQueue.shift();
		ZSSEditor.replaceLocalImageWithRemoteImage(info.id, info.argv0, info.argv1, info.argv2);
	}
};
...
Maleskine.inputEventTimerId = null;
